        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-8">
            {materials.map((material) => (
              <div
                key={material.name}
                className="rounded-3xl border border-[var(--line)] bg-surface backdrop-blur-xl p-8 hover:border-white/20 transition-all"
              >
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                  <div>
                    <h2 className="text-2xl font-bold text-ink mb-2">{material.name}</h2>
                    <p className="text-ink-muted mb-3">{material.description}</p>
                    <span className="inline-block px-3 py-1 text-sm font-semibold rounded-full bg-[#AEDD33]/10 text-accent">
                      {material.durability}
                    </span>
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-6 mt-6">
                  <div>
                    <h3 className="text-sm font-semibold text-ink-muted mb-3 uppercase tracking-wide">Applications</h3>
                    <ul className="space-y-2">
                      {material.applications.map((app) => (
                        <li key={app} className="flex items-start gap-2 text-ink-muted">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-ink-muted mb-3 uppercase tracking-wide">Advantages</h3>
                    <ul className="space-y-2">
                      {material.advantages.map((adv) => (
                        <li key={adv} className="flex items-start gap-2 text-ink-muted">
